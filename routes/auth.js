...
const FACEBOOK_APP_ID = process.env.FACEBOOK_APP_ID;      // add in .env
const FACEBOOK_APP_SECRET = process.env.FACEBOOK_APP_SECRET; // add in .env
// Signup
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (data.is_valid && data.user_id === userID) {
      // Token valid → optionally save user to MongoDB here
      res.status(200).json({ message: "Facebook token verified", user: data });
    } else {
