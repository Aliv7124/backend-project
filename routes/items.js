...
import Item from "../models/Item.js";
import { verifyToken } from "../middleware/authMiddleware.js";
import { io } from "../index.js"; // import io from your main index.js
...
router.post("/", verifyToken, upload.single("image"), async (req, res) => {
  try {
    ...
    // Emit socket notification for new item
    io.emit("newItem", {
      message: `New ${type} item posted: ${name}`,
      item: savedItem,
    });
...
router.put("/:id", verifyToken, async (req, res) => {
  ...
  try {
    ...
    // Emit socket notification for update
    io.emit("updateItem", {
      message: `Item updated: ${updatedItem.name}`,
      item: updatedItem,
    });
